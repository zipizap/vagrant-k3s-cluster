...
vagrantSnapshotSavePreIstio_ifNotYetExists() {
  local snapshotName=PreIstio
  if ! vagrant snapshot list | grep "${snapshotName}" &>/dev/null
  then
    # snapshot does not yet exists
    vagrant snapshot save "${snapshotName}"
  fi

}

vagrantSnapshotRestorePreIstio() {
  local snapshotName=PreIstio
  if vagrant snapshot list | grep "${snapshotName}" &>/dev/null
  then
    # snapshot does not yet exists
    ...
    # snapshot does not exist, unexpected
    vagrant snapshot list
    echo "?? snapshot $snapshotName does not exist?? unexpected, aborting"
    exit 99
  fi
...
main() {
  ...
  #   $0 uninstall
  #
  # restore snapshot and reinstall
  #   $0 restoreSnapshotPreIstio_and_reinstall
  #
  ...
  if [[ "${1:-}" == "restoreSnapshotPreIstio_and_reinstall" ]]
  then
    vagrantSnapshotRestorePreIstio
  fi
